...
def save_house_info():
    ...
    max_days = house_dict.get("max_days")  # 最多入住天数，0表示不限制
...
def save_house_image():
    ...
    image_url = constants.FDFS_SERVER_URL + file_name
...
def get_house_index():
    ...
    if ret:
        ...
        json_houses = json.dumps({"errno": RET.OK, "errmsg": "OK", "data": houses_list})
        try:
...
def get_house_detail(house_id):
    """获取房屋详情"""
    # 前端在房屋镶嵌页面展示时，如果浏览页面的用户不是该房屋的房东，则暂时预定按钮，否则不展示。
...
# GET /api/v1.0/houses?sd=20171201&ed=20171231&aid=10&sk=new&p=1
...
def get_house_list():
    ...
    if conflict_orders:
        # 从订单中获取冲突的房屋id
        conflict_orders_ids = [order.house_id for order in conflict_orders]  # 列表推导式
    ...
    resp_dict = dict(errno=RET.OK, errmsg="OK", data={"houses": houses, "current_page": page, "total_page": total_page})
    ...
    if page <= total_page:
        ...
        try:
            ...
            pipeline.expire(redis_key, constants.HOME_PAGE_DATA_REDIS_EXPIRES)
